...
**Surface split by query shape — get-by-code rides inline in `get_airport`, cross-airport search is its own tool.** Per the skill's "cut the surface": the overwhelmingly common need is "give me this airport with its runways and frequencies," served in one `get_airport` call — separate `get_runways`/`get_frequencies` tools would split a single agent action for no benefit, so runways and frequencies stay inline there. A *different* query shape — runway-attribute **search** ("airports in US-WA with a paved runway ≥ 5,000 ft") — is not get-by-code and earns its own tool, `ourairports_search_runways`. Three decisions shape it: (1) **flat `{ airport, runway }` rows, not grouped-by-airport** — mirrors `find_airports`'s `{ airport, distanceKm, bearingDeg }` shape and keeps `limit`/truncation unambiguous (one `limit` caps total rows; an airport with 3 matching runways contributes 3 rows), where a grouped shape would split `limit` between airports and runways and need its own per-airport truncation; (2) **`surface` is a case-insensitive substring match, not an enum** — the corpus has 654 distinct raw surface strings with no controlled vocabulary (`ASP`/`ASPH`/`Asphalt`/`ASPH-G` all mean asphalt), so an exact/enum filter would silently miss most real queries; (3) **unknown length/width is excluded, never assumed to pass** when `min_length_ft`/`min_width_ft` is set — the server can't claim a runway meets a threshold it has no data for. Navaids get their own tool only because their primary access is *near a coordinate* (independent of any airport), which `get_airport` can't express.
